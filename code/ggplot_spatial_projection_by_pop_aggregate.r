...
png(paste0("./output/ggplot_spatial_projection_by_pop_aggregate",stage,".png"),width=500,height=600)
...
g <- ggplot(sub_pop[sub_pop$pop=="Aggregate",], aes(x = yr,
  ...
  xlab("Brood year")
if(stage=="Spwn")
  g <- g + geom_point(data = odfw_pop_ag, aes(x=spawning_year, y = Estimate),
             inherit.aes = FALSE)
...
#Use this ggarrange after you have attached the individual rear and spwn ggplots to a list call g_list

# ggpubr::ggarrange(g_list$rear,g_list$Spwn, ncol = 1, common.legend = TRUE, legend='right', labels = c('A','B'))
