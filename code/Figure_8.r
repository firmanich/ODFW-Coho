...
med_rivers_Spwn <- c("Coquille", "Nestucca", 'Alsea', 'Middle Umpqua', "Yaquina")
Spwn_list <- list(high_rivers_Spwn = high_rivers_Spwn,
                  med_rivers_Spwn = med_rivers_Spwn)
  ...
  for(yr_i in yrs){
    ...
    for(rr in rivers){
      ...
      if(yr_i%in%unique(obs_data$yr[obs_data$PopGrp==rr])){
        ...
        for(cen_rivers in 1:length(Spwn_list)){
          ...
          ind_with_censored_data <- sdmTMB::get_index(cen_pred) %>% 
            ...
            mutate(data = "censored",
                   cen_river = names(Spwn_list)[cen_rivers],
                   river = rr)
...
river_order <- unique(c(high_rivers_Spwn, med_rivers_Spwn, unique(out$river[!(unique(out$river)%in%c(high_rivers_Spwn,med_rivers_Spwn))])))
# mutate(cen_river_label = factor(cen_river, labels = c("High \nRMSE", "Permanently\nremoved", "Medium \nRMSE"))) %>%
out <- out %>% 
  filter(yr >= 2017) %>% 
  mutate(river_labels = factor(river, levels = river_order)) %>%
  # mutate(cen_label = ifelse(river %in% Spwn_list[[1]], "High RMSE",
  #                           ifelse(river %in% Spwn_list[[2]], "Medium RMSE",
  #                                  "Permanently excluded")))  %>% 
  mutate(cen_label = ifelse(cen_river == names(Spwn_list)[1], paste0("\n",paste0(Spwn_list[[1]][1:2], collapse = ", "),"\n",
                                                                     paste0(Spwn_list[[1]][3], collapse = ", "),"\n",
                                                                     paste0(Spwn_list[[1]][4:5], collapse = ", ")),
                          ifelse(cen_river == names(Spwn_list)[2], paste0(Spwn_list[[2]], collapse = ", "),
                                 "All"))) %>%
  filter(cen_river != c("med_rivers_Spwn") ) #%>%
  # na.omit() %>% 
...
print(g)
ggsave(filename = "./output/Figure_8.tiff", dpi = 300, units = "in", width = 12, height = 10)

#Out of bag samples censored analysis vs full data
out %>% filter(!(river %in% Spwn_list[[1]])) %>% filter(data == "censored") %>% summarize(sd_sd = sqrt(mean((est - sum)^2)))
out %>% filter(!(river %in% Spwn_list[[1]])) %>% filter(data == "full") %>% summarize(sd_sd = sqrt(mean((est - sum)^2)))
...
out %>% filter(!(river %in% Spwn_list[[1]])) %>% filter(data == "censored") %>% summarize(sd_sd = mean(se))
out %>% filter(!(river %in% Spwn_list[[1]])) %>% filter(data == "full") %>% summarize(sd_sd = mean(se))
