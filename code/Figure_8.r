...
if(recreate_analysis){
  ...
  for(yr_i in yrs){
    ...
    for(rr in rivers){#c("Alsea","Coos","Necanicum")){
      print(paste(rr,yr_i))
      ...
      if(yr_i%in%unique(obs_data$yr[obs_data$PopGrp==rr])){
        ...
        for(cen_rivers in 1:length(Spwn_list)){
          ...
          data_after_year_of_interest <- obs_data[obs_data$yr >= min(yrs) & obs_data$yr <= yr_i, ] %>%
            filter((PopGrp %in% Spwn_list[[cen_rivers]])) %>%
...
list_num <- 2
out <- out %>% 
  filter(river %in% c("Alsea", "Coos", "Nehalem")) 
...
g2 <- plot_data %>% 
  ...
  theme_bw() +
  ylab("") +
  xlab("Ave. spawner abund. 2017- 2021") +
  theme(panel.grid = element_blank(), ) +
  theme(text = element_text(size = 12)) +
...
g3 <- plot_data %>% 
  ...
  theme_bw() +
  ylab("") +
  xlab("Annual spawner change 2017- 2021") +
  theme(panel.grid = element_blank(), ) +
  theme(text = element_text(size = 12)) +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0) 

g <- ggpubr::ggarrange(g1,g2,g3,ncol = 3)
print(g)
ggsave(g, file = "output/Figure_8.png", width = 10, height = 10)
