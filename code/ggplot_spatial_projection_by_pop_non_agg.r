...
png(paste0("./output/ggplot_spatial_projection_by_pop",stage,"_non_agg.png"),width=800,height=500)
...
if(stage=="Spwn")
  g <- g +
  geom_point(data = odfw_pop[!odfw_pop$pop%in%c('Sixes','Aggregate'),], aes(x=spawning_year, y = CohoNaturalOriginSpawners),
             inherit.aes = FALSE)
# if(stage=="rear")
#   g <- g +
#   geom_point(data = odfw[odfw$pop!="Aggregate",], aes(x=yr, y = Estimate),
#              inherit.aes = FALSE)
# 
print(g)
