...
for(i in 1:nmods){
  # i <- 1
  train = dplyr::filter(df, JuvYr < (mod_search$args$rf$test_years[i] - mod_search$args$rf$n_years_ahead[i] + 1))  
  test = dplyr::filter(df, JuvYr == mod_search$args$rf$test_years[i]) 
  
  #if this is a projection, get the best model from the exploratory loop
  if(project){
    print(paste("Projection year", mod_search$args$rf$test_years[i], " for ", 
    ...
    print(paste("rf exploration", i, " of ", nmods))
    ...
    fit = randomForest(mod_frm, 
                       ...
                       ntree = ntree,
                       data=train)
    grid_search$rmse[i] = sqrt(mean((fit$predicted-train$Juv.km)^2))
